...
const Home = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const caminhaoResponse = await axios.get(`http://localhost:3000/api/caminhoes/${placa}`);
      setCaminhaoBuscado(caminhaoResponse.data);

      const gastosResponse = await axios.get(`http://localhost:3000/api/gastos/caminhao/${caminhaoResponse.data.id}`);
  ...
  return (
    <div className="flex flex-col items-center p-8 bg-neutral min-h-screen">
      <div className="card w-full max-w-2xl">
        <h1 className="text-3xl font-bold mb-6 text-center text-text-dark">Sistema de Gestão de Transportadora</h1>
        ...
        {error && <div className="bg-accent text-white p-4 mb-4 rounded text-center">{error}</div>}
        ...
        {!loading && !error && caminhaoBuscado && (
          <div className="card w-full mt-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Dados do Caminhão - Placa: {caminhaoBuscado.placa}</h2>
              <div className="flex space-x-2">
                <Link to={`/caminhao/editar/${caminhaoBuscado.placa}`} className="btn-primary">Editar</Link>
                <Link to={`/caminhao/${caminhaoBuscado.placa}`} className="btn-secondary">Ver Detalhes</Link>
              </div>
            </div>
            
            <p className="text-text-dark">KM Atual: {caminhaoBuscado.km_atual}</p>
            <p className="text-text-dark">Quantidade de Pneus: {caminhaoBuscado.qtd_pneus}</p>
            ...
            {gastos.length > 0 ? (
              ...
              <p className="text-gray-500">Nenhum gasto encontrado para este caminhão.</p>
            )}
          </div>
        )}
        ...
        {!loading && !error && placa === '' && caminhoes.length > 0 && (
          <div className="card w-full mt-6">
            <h2 className="text-xl font-bold mb-4">Todos os Caminhões Cadastrados</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-gray-200">
                ...
                <tbody>
                  {caminhoes.map(c => (
                    <tr key={c.id}>
                      <td className="py-2 px-4 border-b text-center">{c.placa}</td>
                      <td className="py-2 px-4 border-b text-center">{c.km_atual}</td>
                      <td className="py-2 px-4 border-b text-center">
                        <Link to={`/caminhao/editar/${c.placa}`} className="btn-primary text-sm px-2 py-1 mr-2">Editar</Link>
                        <Link to={`/caminhao/${c.placa}`} className="btn-secondary text-sm px-2 py-1">Detalhes</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
