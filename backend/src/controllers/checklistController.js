...
export const checklistController = {
  ...
  getChecklistsByCaminhaoId: async (req, res) => {
    try {
      const checklists = await checklistModel.getByCaminhaoId(req.params.caminhaoId);
      res.status(200).json(checklists);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  updateChecklist: async (req, res) => {
    try {
      const checklistAtualizado = await checklistModel.update(req.params.id, req.body);
