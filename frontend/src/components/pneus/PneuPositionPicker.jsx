import React, { useMemo } from "react";
import PropTypes from "prop-types";
import { buildPositionDiagram } from "../../utils/pneuPosicaoMap.js";

const TireButton = ({ label, short, selected, occupied, onClick, title }) => (
  <button
    type="button"
    onClick={occupied && !selected ? undefined : onClick}
    disabled={occupied && !selected}
    title={title || label}
    className={`flex flex-col items-center justify-center min-w-[2.25rem] sm:min-w-[2.5rem] min-h-11 h-11 sm:h-12 rounded-lg border-2 text-[10px] font-semibold transition-all ${
      selected
        ? "border-secondary bg-secondary/15 text-secondary shadow-sm ring-2 ring-secondary/20"
        : occupied
          ? "border-amber-300 bg-amber-50 text-amber-800 cursor-not-allowed opacity-90"
          : "border-border bg-white text-text-secondary hover:border-secondary/50 hover:bg-secondary/5"
    }`}
  >
    <span className="text-base leading-none">⬤</span>
    <span>{short || label}</span>
  </button>
);

TireButton.propTypes = {
  label: PropTypes.string,
  short: PropTypes.string,
  selected: PropTypes.bool,
  occupied: PropTypes.bool,
  onClick: PropTypes.func,
  title: PropTypes.string,
};

const isOccupied = (id, occupiedIds) =>
  occupiedIds.includes(String(id));

const DualAxle = ({
  leftOuter,
  leftInner,
  rightInner,
  rightOuter,
  value,
  occupiedIds,
  onSelect,
}) => (
  <div className="flex items-center justify-center gap-1 sm:gap-2">
    <div className="flex gap-0.5 sm:gap-1">
      {leftInner && (
        <TireButton
          short="EI"
          label={leftInner.nome_posicao}
          selected={value === String(leftInner.id)}
          occupied={isOccupied(leftInner.id, occupiedIds)}
          onClick={() => onSelect(leftInner.id)}
        />
      )}
      {leftOuter && (
        <TireButton
          short="EE"
          label={leftOuter.nome_posicao}
          selected={value === String(leftOuter.id)}
          occupied={isOccupied(leftOuter.id, occupiedIds)}
          onClick={() => onSelect(leftOuter.id)}
        />
      )}
    </div>
    <div className="h-1 w-10 sm:w-16 md:w-24 bg-gray-300 rounded shrink-0" />
    <div className="flex gap-0.5 sm:gap-1">
      {rightOuter && (
        <TireButton
          short="DE"
          label={rightOuter.nome_posicao}
          selected={value === String(rightOuter.id)}
          occupied={isOccupied(rightOuter.id, occupiedIds)}
          onClick={() => onSelect(rightOuter.id)}
        />
      )}
      {rightInner && (
        <TireButton
          short="DI"
          label={rightInner.nome_posicao}
          selected={value === String(rightInner.id)}
          occupied={isOccupied(rightInner.id, occupiedIds)}
          onClick={() => onSelect(rightInner.id)}
        />
      )}
    </div>
  </div>
);

DualAxle.propTypes = {
  leftOuter: PropTypes.object,
  leftInner: PropTypes.object,
  rightInner: PropTypes.object,
  rightOuter: PropTypes.object,
  value: PropTypes.string,
  occupiedIds: PropTypes.arrayOf(PropTypes.string),
  onSelect: PropTypes.func.isRequired,
};

const axleLabel = (axle, index) => {
  if (axle.number >= 2) {
    return `Eixo ${axle.number}`;
  }
  return index === 0 ? "Eixo traseiro 1" : `Eixo traseiro ${index + 1}`;
};

const PneuPositionPicker = ({
  posicoes,
  value,
  onChange,
  error,
  caminhao,
  occupiedIds = [],
  hint,
}) => {
  const diagram = useMemo(
    () => buildPositionDiagram(posicoes, caminhao),
    [posicoes, caminhao],
  );

  const selectedId = value ? String(value) : "";
  const pick = (id) => onChange(id ? String(id) : "");

  const hasFront = diagram.front.left || diagram.front.right;
  const hasAxles = diagram.axles.some(
    (axle) =>
      axle.leftOuter ||
      axle.leftInner ||
      axle.rightInner ||
      axle.rightOuter,
  );

  return (
    <div className="space-y-4">
      <div>
        <p className="text-sm font-medium text-text-secondary mb-1">
          Posição no veículo <span className="text-danger">*</span>
        </p>
        {hint && (
          <p className="text-xs text-secondary font-medium mb-2">{hint}</p>
        )}
        {caminhao && (
          <p className="text-xs text-text-light mb-3">{diagram.layout.description}</p>
        )}

        {!caminhao && (
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 mb-3">
            Selecione o caminhão para exibir o diagrama correto (com ou sem carreta).
          </p>
        )}

        {occupiedIds.length > 0 && (
          <p className="text-xs text-amber-700 mb-2">
            Posições em amarelo já foram atribuídas a outro pneu desta lista.
          </p>
        )}

        <div className="rounded-xl border border-border bg-gray-50/80 p-3 sm:p-4 space-y-5 overflow-x-auto">
          {hasFront && (
            <div>
              <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2">
                Eixo dianteiro
              </p>
              <div className="flex items-center justify-center gap-3 sm:gap-8 min-w-0">
                {diagram.front.left && (
                  <TireButton
                    short="ESQ"
                    label={diagram.front.left.nome_posicao}
                    selected={selectedId === String(diagram.front.left.id)}
                    occupied={isOccupied(diagram.front.left.id, occupiedIds)}
                    onClick={() => pick(diagram.front.left.id)}
                  />
                )}
                <div className="h-1 w-12 sm:w-20 bg-gray-300 rounded shrink-0" />
                {diagram.front.right && (
                  <TireButton
                    short="DIR"
                    label={diagram.front.right.nome_posicao}
                    selected={selectedId === String(diagram.front.right.id)}
                    occupied={isOccupied(diagram.front.right.id, occupiedIds)}
                    onClick={() => pick(diagram.front.right.id)}
                  />
                )}
              </div>
            </div>
          )}

          {diagram.axles.map((axle, index) => {
            const hasTires =
              axle.leftOuter ||
              axle.leftInner ||
              axle.rightInner ||
              axle.rightOuter;

            if (!hasTires) return null;

            return (
              <div key={`axle-${axle.number}`}>
                <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2">
                  {axleLabel(axle, index)}
                </p>
                <DualAxle
                  leftOuter={axle.leftOuter}
                  leftInner={axle.leftInner}
                  rightInner={axle.rightInner}
                  rightOuter={axle.rightOuter}
                  value={selectedId}
                  occupiedIds={occupiedIds}
                  onSelect={pick}
                />
              </div>
            );
          })}

          {diagram.spares.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2">
                Estepes
              </p>
              <div className="flex flex-wrap gap-2 justify-center">
                {diagram.spares.map((spare, index) => (
                  <TireButton
                    key={spare.id}
                    short={`EST ${index + 1}`}
                    label={spare.nome_posicao}
                    selected={selectedId === String(spare.id)}
                    occupied={isOccupied(spare.id, occupiedIds)}
                    onClick={() => pick(spare.id)}
                  />
                ))}
              </div>
            </div>
          )}

          {!hasFront && !hasAxles && diagram.spares.length === 0 && (
            <p className="text-sm text-text-secondary text-center py-4">
              Nenhuma posição disponível para este veículo.
            </p>
          )}
        </div>

        {diagram.unmapped.length > 0 && (
          <div className="mt-3">
            <p className="text-xs text-text-secondary mb-2">
              Posições adicionais (fora do diagrama deste veículo):
            </p>
            <div className="flex flex-wrap gap-2">
              {diagram.unmapped.map((pos) => {
                const occupied = isOccupied(pos.id, occupiedIds);
                const selected = selectedId === String(pos.id);
                return (
                  <button
                    key={pos.id}
                    type="button"
                    disabled={occupied && !selected}
                    onClick={() => pick(pos.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                      selected
                        ? "border-secondary bg-secondary/10 text-secondary"
                        : occupied
                          ? "border-amber-300 bg-amber-50 text-amber-800 cursor-not-allowed"
                          : "border-border bg-white text-text-secondary hover:bg-gray-50"
                    }`}
                  >
                    {pos.nome_posicao}
                  </button>
                );
              })}
            </div>
          </div>
        )}
      </div>

      {error && <p className="text-xs font-medium text-danger">{error}</p>}
    </div>
  );
};

PneuPositionPicker.propTypes = {
  posicoes: PropTypes.arrayOf(PropTypes.object).isRequired,
  value: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  onChange: PropTypes.func.isRequired,
  error: PropTypes.string,
  caminhao: PropTypes.object,
  occupiedIds: PropTypes.arrayOf(PropTypes.string),
  hint: PropTypes.string,
};

export default PneuPositionPicker;

