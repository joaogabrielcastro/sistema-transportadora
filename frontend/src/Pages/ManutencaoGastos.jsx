...
const ManutencaoGastos = () => {
  ...
  const [form, setForm] = useState({
    ...
    observacao: '',
    oficina: '',
    km_registro: '', // Novo campo para quilometragem
    quantidade_combustivel: '', // Novo campo para combustível
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [caminhoesRes, itensRes, tiposRes, gastosRes, checklistRes] = await Promise.all([
        ...
        setRegistros([...gastosFormatados, ...checklistFormatados].sort((a, b) => new Date(b.data) - new Date(a.data)));
      } catch (err) {
    ...
    try {
      if (form.tipo === 'gasto') {
        const payload = {
          ...
          quantidade_combustivel: form.quantidade_combustivel ? parseFloat(form.quantidade_combustivel) : null,
        };
        ...
        await axios.post('http://localhost:3000/api/gastos', {
          ...
          valor: parseFloat(form.valor),
          data_gasto: form.data,
          descricao: `Manutenção: ${itensChecklist.find(item => item.id === parseInt(form.tipo_id))?.nome_item || ''} - ${form.observacao}`,
        });
      }
  ...
  const handleDelete = async (tipo, id) => {
    if (window.confirm('Tem certeza que deseja deletar este registro?')) {
      try {
        ...
        setRegistros(registros.filter(r => !(r.id === id && r.tipo_registro === tipo)));
      } catch (err) {
  ...
  return (
    <div className="p-8 bg-neutral min-h-screen">
      <div className="card max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-center text-text-dark">Manutenção e Gastos</h1>
        ...
        <div className="card mb-8">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <select name="tipo" value={form.tipo} onChange={handleTipoChange} required className="input">
            ...
            <div>
              ...
              <select name="caminhao_id" value={form.caminhao_id} onChange={handleChange} required className="input">
                <option value="">Selecione o Caminhão</option>
                {caminhoes.map(c => <option key={c.id} value={c.id}>{c.placa}</option>)}
              </select>
            </div>
            <div>
              <label className="label">{form.tipo === 'gasto' ? 'Tipo de Gasto' : 'Item de Manutenção'}</label>
              <select name="tipo_id" value={form.tipo_id} onChange={handleChange} required className="input">
                <option value="">Selecione...</option>
                {form.tipo === 'gasto' ? (
                  tiposGastos.map(t => <option key={t.id} value={t.id}>{t.nome_tipo}</option>)
                ) : (
                  itensChecklist.map(i => <option key={i.id} value={i.id}>{i.nome_item}</option>)
                )}
              </select>
            </div>
            <div>
              <label className="label">Valor</label>
              <input type="number" name="valor" value={form.valor} onChange={handleChange} className="input" step="0.01" required />
            </div>
            {form.tipo === 'manutencao' && (
              <div>
                <label className="label">Oficina</label>
                <input type="text" name="oficina" value={form.oficina} onChange={handleChange} className="input" />
              </div>
            )}
            {form.tipo === 'gasto' && form.tipo_id === 1 && ( // ID do tipo de gasto 'Combustível'
              <div>
                <label className="label">Quantidade (Litros)</label>
                <input type="number" name="quantidade_combustivel" value={form.quantidade_combustivel} onChange={handleChange} className="input" step="0.01" required />
              </div>
            )}
            <div>
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              <textarea name="observacao" value={form.observacao} onChange={handleChange} className="input"></textarea>
            </div>
            {/* O campo de KM de registro será adicionado aqui, mas não é obrigatório para todos os tipos */}
            <div>
              <label className="label">Quilometragem (KM)</label>
              <input type="number" name="km_registro" value={form.km_registro} onChange={handleChange} className="input" />
            </div>
          ...
          {registros.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-gray-200">
                ...
                <tbody>
                  {registros.map((r) => (
                    <tr key={`${r.tipo_registro}-${r.id}`} className="border-b hover:bg-neutral transition-colors duration-200">
                      <td className="py-2 px-4 text-center">{r.tipo_registro}</td>
                      <td className="py-2 px-4 text-center">{r.placa || 'N/A'}</td>
                      <td className="py-2 px-4 text-center">{r.nome_tipo || 'N/A'}</td>
                      <td className="py-2 px-4 text-center">{r.oficina || 'N/A'}</td>
                      <td className="py-2 px-4 text-center">{r.valor !== 'N/A' ? `R$ ${r.valor}` : r.valor}</td>
                      <td className="py-2 px-4 text-center">{r.data}</td>
                      <td className="py-2 px-4 text-center">{r.km_registro || 'N/A'}</td>
                      <td className="py-2 px-4 text-center">
                        <button onClick={() => handleDelete(r.tipo_registro, r.id)} className="text-accent hover:underline">
                          Deletar
                        </button>
                      </td>
                    </tr>
